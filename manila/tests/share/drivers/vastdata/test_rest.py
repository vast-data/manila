...
from manila.share.drivers.vastdata.rest import Session
from manila.share.drivers.vastdata.rest import VastApiException
from manila.share.drivers.vastdata.rest import VastResource
...
class TestSession(unittest.TestCase):
    ...
    def setUp(self):
        self.session = Session("host", "username", "password", False, "1.0")
    ...
    def test_refresh_auth_token_failure(self, mock_request):
        ...
        with self.assertRaises(VastApiException):
            self.session.refresh_auth_token()
    ...
    def test_request_failure_500(self):
        ...
        with patch("requests.Session.request", new=lambda *a, **k: resp):
            with self.assertRaises(VastApiException) as exc:
    ...
    def test_refresh_token_retries(self):
        ...
        resp.raw = BytesIO(b"Token is invalid")
...
class TestVastResource(unittest.TestCase):
    def setUp(self):
        ...
        self.vast_resource = VastResource(self.mock_rest)
    ...
    def test_delete_when_entry_not_found(self):
        self.vast_resource.one = MagicMock(return_value=None)
    ...
    def test_delete_when_entry_found(self):
        ...
        mock_entry.id = "1"
        self.vast_resource.one = MagicMock(return_value=mock_entry)
        self.vast_resource.delete("test")
    ...
    def test_one_when_no_entries_found(self):
        self.vast_resource.list = MagicMock(return_value=[])
        result = self.vast_resource.one("test")
        self.assertIsNone(result)

    def test_one_when_multiple_entries_found(self):
        self.vast_resource.list = MagicMock(
            return_value=[MagicMock(), MagicMock()]
        )
        with self.assertRaises(ShareBackendException):
            self.vast_resource.one("test")

    def test_one_when_single_entry_found(self):
        mock_entry = MagicMock()
        self.vast_resource.list = MagicMock(return_value=[mock_entry])
        result = self.vast_resource.one("test")
    ...
    def test_ensure_when_entry_not_found(self):
        self.vast_resource.one = MagicMock(return_value=None)
        mock_entry = MagicMock()
        self.vast_resource.create = MagicMock(return_value=mock_entry)
...
class TestFolders(unittest.TestCase):
    ...
    def setUp(self):
        self.rest_api = RestApi("host", "username", "password", True, "1.0")
    ...
    def test_trash_api_disabled(self):
        def raise_http_err(*args, **kwargs):
            ...
            raise VastApiException(message=resp.text)
        ...
        with (
            ...
            with self.assertRaises(ShareBackendException) as exc:
...
class TestRestApi(unittest.TestCase):
    ...
    @patch("manila.share.drivers.vastdata.rest.Session")
    def test_get_sw_version(self, mock_session):
        mock_session.return_value.versions.return_value = [
            MagicMock(sys_version="1.0")
        ]
        rest_api = RestApi("host", "username", "password", True, "1.0")
