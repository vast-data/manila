...
class VASTShareDriver(driver.ShareDriver):
    ...
    def do_setup(self, context):
        ...
        vip_pool_name = self.configuration.vast_vippool_name
        ...
        port = self.configuration.vast_mgmt_port
        if port:
            host = f"{host}:{port}"
    ...
    def ensure_share(self, context, share, share_server=None):
        ...
        if quota.hard_limit != requested_capacity:
            raise exception.ManilaException(
                f"Share already exists with different capacity"
    ...
    def update_access(
        ...
        if not policy:
            raise exception.ManilaException(
                f"Policy not found for share {share_id}"
            )
        if add_rules:
            policy_rules = policy_payload_from_rules(
                rules=add_rules, policy=policy, action="update"
            )
